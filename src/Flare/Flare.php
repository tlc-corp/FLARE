...
namespace Flare {
	...
	if(\Phar::running(true) !== ""){
		@define('Flare\PATH', \Phar::running(true) . "/");
	}else{
		@define('Flare\PATH', \getcwd() . DIRECTORY_SEPARATOR);
	}
	...
	if(!class_exists("ClassLoader", false)){
		require_once(\Flare\PATH . "src/spl/ClassLoader.php");
		require_once(\Flare\PATH . "src/spl/BaseClassLoader.php");
		require_once(\Flare\PATH . "src/Flare/CompatibleClassLoader.php");
	}

	$autoloader = new CompatibleClassLoader();
	$autoloader->addPath(\Flare\PATH . "src");
	$autoloader->addPath(\Flare\PATH . "src" . DIRECTORY_SEPARATOR . "spl");
	...
	define('Flare\START_TIME', microtime(true));

	$opts = getopt("", ["data:", "plugins:", "no-wizard", "enable-profiler"]);

	define('Flare\DATA', isset($opts["data"]) ? $opts["data"] . DIRECTORY_SEPARATOR : \getcwd() . DIRECTORY_SEPARATOR);
	define('Flare\PLUGIN_PATH', isset($opts["plugins"]) ? $opts["plugins"] . DIRECTORY_SEPARATOR : \getcwd() . DIRECTORY_SEPARATOR . "plugins" . DIRECTORY_SEPARATOR);

	Terminal::init();

	define('Flare\ANSI', Terminal::hasFormattingCodes());

	if(!file_exists(\Flare\DATA)){
		mkdir(\Flare\DATA, 0777, true);
	}

	//Logger has a dependency on timezone, so we'll set it to UTC until we can get the actual timezone.
	date_default_timezone_set("UTC");

	$logger = new MainLogger(\Flare\DATA . "server.log", \Flare\ANSI);
	...
	function cleanPath($path){
		return rtrim(str_replace(["\\", ".php", "phar://", rtrim(str_replace(["\\", "phar://"], ["/", ""], \Flare\PATH), "/"), rtrim(str_replace(["\\", "phar://"], ["/", ""], \Flare\PLUGIN_PATH), "/")], ["/", "", "", "", ""], $path), "/");
	}

	$errors = 0;

	if(php_sapi_name() !== "cli"){
		$logger->critical("You must run Flare using the CLI.");
		++$errors;
	}
	...
	if(extension_loaded("Flare")){
		if(version_compare(phpversion("Flare"), "0.0.1") < 0){
			$logger->critical("You have the native Flare extension, but your version is lower than 0.0.1.");
			++$errors;
		}elseif(version_compare(phpversion("Flare"), "0.0.4") > 0){
			$logger->critical("You have the native Flare extension, but your version is higher than 0.0.4.");
			++$errors;
		}
	}
	...
	if(file_exists(\Flare\PATH . ".git/refs/heads/master")){ //Found Git information!
		define('Flare\GIT_COMMIT', strtolower(trim(file_get_contents(\Flare\PATH . ".git/refs/heads/master"))));
	}else{ //Unknown :(
		define('Flare\GIT_COMMIT', str_repeat("00", 20));
	}
	...
	$lang = "unknown";
	if(!file_exists(\Flare\DATA . "server.properties") and !isset($opts["no-wizard"])){
		$inst = new Installer();
		$lang = $inst->getDefaultLang();
	}

	/*if(\Phar::running(true) === ""){
		$logger->warning("Non-packaged Flare installation detected, do not use on production.");
	}*/

	ThreadManager::init();
	$server = new Server($autoloader, $logger, \Flare\PATH, \Flare\DATA, \Flare\PLUGIN_PATH, $lang);
