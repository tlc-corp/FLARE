...
use Flare\block\PressurePlate;
use Flare\command\CommandSender;
use Flare\entity\Animal;
use Flare\entity\Arrow;
use Flare\entity\Attribute;
use Flare\entity\AttributeMap;
...
use Flare\entity\Living;
use Flare\entity\Minecart;
use Flare\entity\Projectile;
use Flare\entity\ThrownExpBottle;
use Flare\entity\ThrownPotion;
use Flare\event\block\BlockBreakEvent;
use Flare\event\block\ItemFrameDropItemEvent;
use Flare\event\block\SignChangeEvent;
use Flare\event\entity\EntityDamageByBlockEvent;
use Flare\event\entity\EntityDamageByEntityEvent;
use Flare\event\entity\EntityDamageEvent;
use Flare\event\entity\EntityEatItemEvent;
use Flare\event\entity\EntityRegainHealthEvent;
use Flare\event\entity\EntityShootBowEvent;
use Flare\event\entity\ProjectileLaunchEvent;
use Flare\event\inventory\CraftItemEvent;
use Flare\event\inventory\InventoryCloseEvent;
use Flare\event\inventory\InventoryPickupArrowEvent;
use Flare\event\inventory\InventoryPickupItemEvent;
use Flare\event\player\PlayerTextPreSendEvent;
use Flare\event\player\PlayerAchievementAwardedEvent;
use Flare\event\player\PlayerAnimationEvent;
use Flare\event\player\PlayerBedEnterEvent;
use Flare\event\player\PlayerBedLeaveEvent;
use Flare\event\player\PlayerChatEvent;
use Flare\event\player\PlayerCommandPreprocessEvent;
use Flare\event\player\PlayerDeathEvent;
use Flare\event\player\PlayerDropItemEvent;
use Flare\event\player\PlayerExperienceChangeEvent;
use Flare\event\player\PlayerGameModeChangeEvent;
use Flare\event\player\PlayerHungerChangeEvent;
use Flare\event\player\PlayerInteractEvent;
use Flare\event\player\PlayerItemConsumeEvent;
use Flare\event\player\PlayerJoinEvent;
use Flare\event\player\PlayerKickEvent;
use Flare\event\player\PlayerLoginEvent;
use Flare\event\player\PlayerMoveEvent;
use Flare\event\player\PlayerPreLoginEvent;
use Flare\event\player\PlayerQuitEvent;
use Flare\event\player\PlayerRespawnEvent;
use Flare\event\player\PlayerToggleSneakEvent;
use Flare\event\player\PlayerToggleSprintEvent;
use Flare\event\player\PlayerUseFishingRodEvent;
use Flare\event\server\DataPacketReceiveEvent;
use Flare\event\server\DataPacketSendEvent;
use Flare\event\TextContainer;
use Flare\event\Timings;
use Flare\event\TranslationContainer;
use Flare\inventory\AnvilInventory;
use Flare\inventory\BaseTransaction;
use Flare\inventory\BigShapedRecipe;
use Flare\inventory\BigShapelessRecipe;
use Flare\inventory\EnchantInventory;
use Flare\inventory\FurnaceInventory;
use Flare\inventory\Inventory;
use Flare\inventory\InventoryHolder;
use Flare\inventory\PlayerInventory;
use Flare\inventory\ShapedRecipe;
use Flare\inventory\ShapelessRecipe;
use Flare\inventory\SimpleTransactionGroup;
use Flare\item\FoodSource;
use Flare\item\Item;
use Flare\item\Potion;
use Flare\level\ChunkLoader;
use Flare\level\format\FullChunk;
use Flare\level\Level;
use Flare\level\Location;
use Flare\level\Position;
use Flare\level\sound\LaunchSound;
use Flare\math\AxisAlignedBB;
use Flare\math\Vector2;
use Flare\math\Vector3;
use Flare\metadata\MetadataValue;
use Flare\nbt\NBT;
use Flare\nbt\tag\ByteTag;
use Flare\nbt\tag\CompoundTag;
use Flare\nbt\tag\DoubleTag;
...
use Flare\permission\PermissibleBase;
use Flare\permission\PermissionAttachment;
use Flare\plugin\Plugin;
use Flare\tile\ItemFrame;
use Flare\tile\Sign;
use Flare\tile\Spawnable;
use Flare\tile\Tile;
...
class Player extends Human implements CommandSender, InventoryHolder, ChunkLoader, IPlayer{
	...
	protected function checkBlockCollision(){
		...
		if($this->getServer()->redstoneEnabled){
			/** @var \Flare\block\PressurePlate $block * */
		...
		if(!$revert and $distanceSquared != 0){
			...
			/*if($this->isSurvival()){
				if(!$revert and !$this->isSleeping()){
					if($diff > 0.0625){
						$revert = true;
						$this->server->getLogger()->warning($this->getServer()->getLanguage()->translateString("Flare.player.invalidMove", [$this->getName()]));
					}
				}
			}
		...
		if(!$revert and ($delta > (1 / 16) or $deltaAngle > 10)){
			...
			if(!$isFirst){
				...
				if(!($revert = $ev->isCancelled())){ //Yes, this is intended
					...
					/*if($this->server->expEnabled){
						/** @var \Flare\entity\ExperienceOrb $e *
	...
	protected function processLogin(){
		...
		$this->dataPacket($pk);
		$this->server->getLogger()->info($this->getServer()->getLanguage()->translateString("Flare.player.logIn", [
			...
			case ProtocolInfo::INTERACT_PACKET:
				...
				if($target instanceof Entity and $this->getGamemode() !== Player::VIEW and $this->isAlive() and $target->isAlive()){
					if($target instanceof DroppedItem or $target instanceof Arrow){
						...
						$this->server->getLogger()->warning($this->getServer()->getLanguage()->translateString("Flare.player.invalidEntity", [$this->getName()]));
						break;
					}
	...
	public function sendTranslation($message, array $parameters = []){
		...
		if(!$this->server->isLanguageForced()){
			...
			$pk->message = $this->server->getLanguage()->translateString($message, $parameters, "Flare.");
			foreach($parameters as $i => $p){
				$parameters[$i] = $this->server->getLanguage()->translateString($p, $parameters, "Flare.");
			}
	...
	/*public final function setTransferred($transferredTo = "") {
		if ($this->connected and !$this->closed) {
			...
			$this->server->getPluginManager()->unsubscribeFromPermission(Server::BROADCAST_CHANNEL_USERS, $this);
			$this->spawned = false;
			$this->server->getLogger()->info($this->getServer()->getLanguage()->translateString("Flare.player.transferred", [
	...
	public final function close($message = "", $reason = "generic reason", $notify = true){
		...
		if($this->connected and !$this->closed){
			...
			$this->server->getPluginManager()->unsubscribeFromPermission(Server::BROADCAST_CHANNEL_USERS, $this);
			$this->spawned = false;
			$this->server->getLogger()->info($this->getServer()->getLanguage()->translateString("Flare.player.logOut", [
				TextFormat::AQUA . $this->getName() . TextFormat::WHITE,
				$this->ip,
				$this->port,
