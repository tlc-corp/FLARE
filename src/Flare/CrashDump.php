...
class CrashDump{
	...
	private function pluginsData(){
		if(class_exists("Flare\\plugin\\PluginManager", false)){
			$this->addLine();
	...
	private function extraData(){
		...
		if($this->server->getProperty("auto-report.send-settings", true) !== false){
			...
			$this->data["Flare.yml"] = @file_get_contents($this->server->getDataPath() . "Flare.yml");
		}else{
			$this->data["Flare.yml"] = "";
	...
	private function baseCrash(){
		...
		if(strpos($error["file"], "src/Flare/") === false and strpos($error["file"], "src/raklib/") === false and file_exists($error["fullFile"])){
			$this->addLine();
			...
			foreach($this->server->getPluginManager()->getPlugins() as $plugin){
				$filePath = \Flare\cleanPath($file->getValue($plugin));
	...
	private function generalData(){
		...
		$this->data["general"]["api"] = \Flare\API_VERSION;
		$this->data["general"]["git"] = \Flare\GIT_COMMIT;
