...
class FillCommand extends VanillaCommand{
	...
	public function __construct($name){
		parent::__construct(
			$name,
			"%Flare.command.fill.description",
			"%commands.fill.usage"
		);
		$this->setPermission("Flare.command.fill");
	}
		...
		for($a = 0; $a < 6; $a++){
			if(isset($args[$a])){
				if(is_integer($args[$a])){
					...
					$sender->sendMessage(TextFormat::RED . new TranslationContainer("Flare.command.fill.invalidBlock", []));
					return false;
				}
