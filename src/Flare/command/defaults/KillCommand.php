...
class KillCommand extends VanillaCommand{
	...
	public function __construct($name){
		parent::__construct(
			$name,
			"%Flare.command.kill.description",
			"%Flare.command.kill.usage",
			["suicide"]
		);
		$this->setPermission("Flare.command.kill.self;Flare.command.kill.other");
	}
	...
	public function execute(CommandSender $sender, $currentAlias, array $args){
		...
		if(count($args) === 1){
			if(!$sender->hasPermission("Flare.command.kill.other")){
		...
		if($sender instanceof Player){
			if(!$sender->hasPermission("Flare.command.kill.self")){
				$sender->sendMessage(new TranslationContainer(TextFormat::RED . "%commands.generic.permission"));
