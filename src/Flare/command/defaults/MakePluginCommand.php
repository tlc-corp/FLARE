...
class MakePluginCommand extends VanillaCommand{
	...
	public function __construct($name){
		...
		$this->setPermission("Flare.command.makeplugin");
	}
	...
	public function execute(CommandSender $sender, $commandLabel, array $args){
		...
		if($description->getName() === "DevTools"){
			...
			$phar->setStub('<?php echo "Flare/Genisys plugin ' . $description->getName() . ' v' . $description->getVersion() . '\nThis file has been generated using Genisys by iTX Technologies at ' . date("r") . '\n----------------\n";if(extension_loaded("phar")){$phar = new \Phar(__FILE__);foreach($phar->getMetadata() as $key => $value){echo ucfirst($key).": ".(is_array($value) ? implode(", ", $value):$value)."\n";}} __HALT_COMPILER();');
		}
		$phar->setSignatureAlgorithm(\Phar::SHA1);
		$reflection = new \ReflectionClass("Flare\\plugin\\PluginBase");
