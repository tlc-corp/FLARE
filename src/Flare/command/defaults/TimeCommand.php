...
class TimeCommand extends VanillaCommand{
	...
	public function __construct($name){
		parent::__construct(
			$name,
			"%Flare.command.time.description",
			"%Flare.command.time.usage"
		);
		$this->setPermission("Flare.command.time.add;Flare.command.time.set;Flare.command.time.start;Flare.command.time.stop");
	}
	...
	public function execute(CommandSender $sender, $currentAlias, array $args){
		...
		if($args[0] === "start"){
			if(!$sender->hasPermission("Flare.command.time.start")){
			...
			if(!$sender->hasPermission("Flare.command.time.stop")){
				$sender->sendMessage(new TranslationContainer(TextFormat::RED . "%commands.generic.permission"));
			...
			if(!$sender->hasPermission("Flare.command.time.query")){
				$sender->sendMessage(new TranslationContainer(TextFormat::RED . "%commands.generic.permission"));
		...
		if($args[0] === "set"){
			if(!$sender->hasPermission("Flare.command.time.set")){
				$sender->sendMessage(new TranslationContainer(TextFormat::RED . "%commands.generic.permission"));
			...
			if(!$sender->hasPermission("Flare.command.time.add")){
				$sender->sendMessage(new TranslationContainer(TextFormat::RED . "%commands.generic.permission"));
