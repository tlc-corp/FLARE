...
class BiomeCommand extends VanillaCommand{
	...
	public function __construct($name){
		parent::__construct(
			$name,
			"%Flare.command.biome.description",
			"/biome <pos1|pos2|get|set|color>"
		);
		$this->setPermission("Flare.command.biome");
	}
			...
			if($args[0] == "set"){
				...
				if(isset($sender->selectedPos[0]) and isset($sender->selectedPos[1])){
					if(is_numeric($biome) === false){
						$sender->sendMessage(TextFormat::RED . "%Flare.command.biome.wrongBio");
						return false;
					}
					$biome = (int) $biome;
					if($sender->selectedLev[0] !== $sender->selectedLev[1]){
						$sender->sendMessage(TextFormat::RED . "%Flare.command.biome.wrongLev");
						return false;
					}
					...
					$sender->sendMessage(new TranslationContainer("Flare.command.biome.set", [$biome]));
				}else{
					$sender->sendMessage("%Flare.command.biome.noPos");
				}
			}elseif($args[0] == "color"){
				...
				//var_dump($a);
				if(count($a) != 3){
					$sender->sendMessage(TextFormat::RED . "%Flare.command.biome.wrongCol");
					return false;
				}
				if(isset($sender->selectedPos[0]) and isset($sender->selectedPos[1])){
					if($sender->selectedLev[0] !== $sender->selectedLev[1]){
					...
					$sender->sendMessage(new TranslationContainer("Flare.command.biome.color", [$a[0], $a[1], $a[2]]));
				}else{
					$sender->sendMessage("%Flare.command.biome.noPos");
				}
			}elseif($args[0] == "pos1"){
				...
				$sender->sendMessage(new TranslationContainer("Flare.command.biome.posset", [$sender->selectedLev[0]->getName(), $x, $z, "1"]));
			}elseif($args[0] == "pos2"){
				$x = (int) $sender->getX();
				$z = (int) $sender->getZ();
				...
				$sender->sendMessage(new TranslationContainer("Flare.command.biome.posset", [$sender->selectedLev[1]->getname(), $x, $z, "2"]));
			}elseif($args[0] == "get"){
				$x = (int) $sender->getX();
				$z = (int) $sender->getZ();
				$biome = $sender->getLevel()->getBiomeId($x, $z);
				$color = $sender->getLevel()->getBiomeColor($x, $z);
				$sender->sendMessage(new TranslationContainer("Flare.command.biome.get", [$biome, $color[0], $color[1], $color[2]]));
			}else{
				$sender->sendMessage(new TranslationContainer("commands.generic.usage", [$this->usageMessage]));
				return true;
			}
