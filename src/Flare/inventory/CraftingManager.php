...
use Flare\block\Planks;
use Flare\block\Quartz;
use Flare\block\Sandstone;
use Flare\block\Slab;
use Flare\block\Stone;
use Flare\block\StoneBricks;
use Flare\block\StoneWall;
use Flare\block\Wood;
use Flare\block\Wood2;
use Flare\item\Fish;
use Flare\item\Item;
...
class CraftingManager{
	...
	public function __construct(bool $useJson = false){
		...
		if($useJson){
			// load recipes from src/Flare/recipes.json
			$recipes = new Config(Server::getInstance()->getFilePath() . "src/Flare/resources/recipes.json", Config::JSON, []);
...
*/
	...
	private function createOneIngedientRecipe($recipeshape, $resultitem, $resultitemmeta, $resultitemamound, $ingedienttype, $ingredientmeta, $ingredientname, $inventoryType = ""){
		...
		if($height < 3){
			...
			$fullClassName = "Flare\\inventory\\" . $inventoryType . "ShapedRecipe";// $ShapeClass."ShapedRecipe";
			...
			// Process big recipe
			$fullClassName = "Flare\\inventory\\" . $inventoryType . "BigShapedRecipe";
			$recipe = ((new $fullClassName(Item::get($resultitem, $resultitemmeta, $resultitemamound),
				...$recipeshape
			))->setIngredient($ingredientname, Item::get($ingedienttype, $ingredientmeta, $ingredientamount)));
		}
